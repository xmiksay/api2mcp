...
import type { AuthProviderView, PackAuthProvider } from "./types";
...
export const authProvidersApi = {
  ...
  create: (slug: string, def: PackAuthProvider): Promise<AuthProviderView> =>
    api.post("/api/auth_providers", { slug, ...def }),
  update: (slug: string, def: PackAuthProvider): Promise<AuthProviderView> =>
