//! MCP server: two hand-rolled JSON-RPC 2.0 endpoints.
//!
//! **`POST /mcp/{endpoint_slug}`** is the data plane, unchanged from before this module split:
//! a curated set of api_calls/scripts, tag-selected by one `endpoint` row, served as MCP tools
//! to whatever agent consumes them.
//!
//! **Bare `POST /mcp`** is the control plane ([`control`]): the factory an authoring agent
//! (typically Claude Code) talks to in order to define the services/api_calls/scripts/endpoints
//! the data plane then serves. It used to resolve to `cfg.default_endpoint` — an alias for one
//! particular curated endpoint — but a factory's tools must never sit in the context of an agent
//! that only wants to call `get-item` (`plan.md`'s Fáze 5), so it is now its own surface with its
//! own fixed tool set, reachable only by a token minted with `control_plane: true`
//! (`server::auth::ResolvedCredential`, `store::ServiceTokenRecord::control_plane`). See
//! [`control`]'s own module doc for the tool surface, the ownership/ordering rules, and the I5
//! accepted trade-offs specific to letting an agent define arbitrary origins.
//!
//! [`handle`]/[`handle_control_plane`] take raw [`Bytes`], not `Json<T>`: a malformed body must
//! come back as a properly framed JSON-RPC `-32700` ([`rpc::parse_request`]), not axum's
//! bare-text `400` an ordinary extractor would produce on the same input. Authentication also
//! runs *inside* these handlers rather than in a middleware, for the same reason — a missing/bad
//! credential becomes a framed JSON-RPC `401` carrying `WWW-Authenticate`, not axum's plain-text
//! rejection.
//!
//! [`handlers`] owns the data plane's `tools/list`/`tools/call` (the two methods that need a
//! resolved [`crate::resolve::EndpointPlan`]); [`registry`] renders a plan into MCP's
//! tool-descriptor shape; [`invoke`] is the `invoke`/`list_tools` dispatcher; [`rpc`] is the wire
//! format shared by both surfaces; [`instructions`] is the data plane's `initialize` static text.
//!
//! **Endpoint grants.** [`resolve_plan`] is the one place an endpoint slug turns into a
//! plan for every data-plane method that needs one (`initialize`, `tools/list`, `tools/call`), so
//! it is also the one place a service token's endpoint restriction
//! (`server::auth::ResolvedCredential::endpoint_grants`) is enforced — not a per-handler check a
//! later sibling method could forget. A restricted token naming some other slug gets exactly
//! [`build_endpoint_not_found`]'s response: the same shape [`crate::resolve::build_plan`] itself
//! produces for a slug with no row at all. That indistinguishability is the point — a
//! `403`-shaped "exists but you can't reach it" would tell an attacker which slugs to go after.
//! [`resolve_control_plane`] applies the identical reasoning to the `control_plane` capability: a
//! token without it gets [`build_endpoint_not_found`]'s response too, not a distinct "forbidden".
...
use crate::server::auth::{BearerChallenge, EndpointGrants, ResolvedCredential, authenticate_mcp};
...
/// The pseudo-slug bare `/mcp` is treated as, purely for building the identical-to-"unknown
/// endpoint" refusal a `control_plane`-less credential gets (see [`resolve_control_plane`]) —
/// never looked up against `endpoints`, since bare `/mcp` is never resolved as one.
const CONTROL_PLANE_PSEUDO_SLUG: &str = "mcp";
...
pub fn router() -> Router<AppState> {
    Router::new()
        .route("/mcp", post(handle_control_plane))
...
async fn handle(
    ...
    let credential = match authenticate_mcp(&state.db, &state.cfg, &headers).await {
        Ok(c) => c,
...
/// Bare `POST /mcp` — the control plane. Authentication and JSON-RPC framing mirror [`handle`]
/// exactly (same [`authenticate_mcp`], same [`parse_request`], same `notifications/initialized`
/// short-circuit); what differs is that there is no endpoint slug to resolve, only the
/// [`resolve_control_plane`] capability check [`control::dispatch_method`] runs internally for
/// its three real methods, the same way [`resolve_plan`] runs internally on the data plane.
async fn handle_control_plane(
    ...
    let resp = control::dispatch_method(&state, &credential, req).await;
    (StatusCode::OK, Json(resp)).into_response()
}
...
/// Checks [`ResolvedCredential::control_plane`] for every method that actually does something
/// (`dispatch_method`'s three real arms all funnel through here via [`handle_control_plane`]) —
/// a `Method not found` on an unrelated method never reaches this check, mirroring how
/// [`resolve_plan`]'s own grant check only ever runs for `initialize`/`tools/list`/`tools/call`.
/// A credential lacking the capability gets exactly [`build_endpoint_not_found`]'s response for
/// [`CONTROL_PLANE_PSEUDO_SLUG`] — indistinguishable from a token restricted off some other
/// endpoint, which is indistinguishable from that endpoint never having existed at all. Same
/// reasoning throughout: a capability you lack must never be observable as different from a
/// surface that isn't there.
fn resolve_control_plane(
    credential: &ResolvedCredential,
    id: &Option<Value>,
) -> Result<(), JsonRpcResponse> {
    if credential.control_plane {
        return Ok(());
    }
    // `CONTROL_PLANE_PSEUDO_SLUG` is a hardcoded, syntactically valid `Slug` literal — parsing it
    // can never fail, so this is provably unreachable rather than a real error path.
    let slug: Slug = CONTROL_PLANE_PSEUDO_SLUG
        .parse()
        .expect("\"mcp\" is a valid slug literal");
    Err(build_endpoint_not_found(id, &slug))
}
...
mod tests {
    ...
    fn credential(control_plane: bool) -> ResolvedCredential {
        ResolvedCredential {
            caller: Caller {
                kind: crate::server::identity::CallerKind::ServiceToken,
                id: uuid::Uuid::nil(),
            },
    ...
    #[test]
    fn resolve_control_plane_passes_a_capable_credential() {
        assert!(resolve_control_plane(&credential(true), &None).is_ok());
    }

    /// Pins the whole point of [`resolve_control_plane`]: a token lacking the capability gets
    /// exactly what a restricted data-plane token gets for an endpoint outside its grants —
    /// [`build_endpoint_not_found`] for [`CONTROL_PLANE_PSEUDO_SLUG`], never a distinct shape.
    #[test]
    fn resolve_control_plane_refuses_identically_to_an_unknown_endpoint() {
        let err = resolve_control_plane(&credential(false), &Some(json!(1))).unwrap_err();
        let slug: Slug = CONTROL_PLANE_PSEUDO_SLUG.parse().unwrap();
        let expected = build_endpoint_not_found(&Some(json!(1)), &slug);
        assert_eq!(
